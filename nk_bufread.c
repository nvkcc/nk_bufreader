...
inline int nk_buf_reader_bytes_to_read(nk_buf_reader *r) {
    // Say we have a buffer of length 16, and the `end_of_buf` points to
...
int nk_buf_reader_read_data(nk_buf_reader *r) {
    ...
    // Update the `end_of_buf` address. Note that after this advancement, it
...
int nk_buf_reader_next(nk_buf_reader *r) {
    ...
    if (r->newl) {
        ...
        // Update the invalidated states of r->newl and r->end_of_buf.
        r->end_of_buf -= diff;
